...
const SEED_HMAC_KEY = crypto.createSecretKey(Buffer.from('2QM9JyDhE0YLh4yVLnjD'));
...
export function seedAccountName(seed: string): string {
  return genAccountName(crypto.createHmac('sha1', SEED_HMAC_KEY).update(seed).digest());
}
...
function genAccountName(bytes: Buffer): string {
  ...
  results[0] = EOS_NAME_FIRST_SYMBOL_CHARS.charAt(bytes[0] % EOS_NAME_FIRST_SYMBOL_CHARS.length);

  for (let i = 1, l = results.length; i < l; i++) {
    results[i] = EOS_NAME_BASIC_CHARS.charAt(bytes[i] % EOS_NAME_BASIC_CHARS.length);
  }
