...
export class FarmersWorld {
  ...
  async getAccountStats(
    userAccount?: string,
  ): Promise<{ balance: Balance; energy: Energy }> {
    const res = await this.bot.wax.rpc.get_table_rows({
      code: 'farmersworld',
      scope: 'farmersworld',
      table: 'accounts',
    ...
    for (const bal of row.balances) {
      // "16.6540 WOOD", "16.6540 GOLD", "16.6540 FOOD"
  ...
  async getExchangeInfo(
    ...
    // баланс валют
    const tokens: { [key: string]: eosCommon.ExtendedAsset } = (
      await this.bot.wax.getBalance({
        tokens: ['FWG', 'FWW', 'FWF'],
      })
    ).reduce((total, cur) => {
    ...
    if (fwg && fwg.amount.gt(0)) {
      if (tokens['FWG']?.quantity.isGreaterThanOrEqual(fwg)) {
        ...
        if (!tokens['FWG'])
          tokens['FWG'] = new eosCommon.ExtendedAsset(
            0,
            new eosCommon.ExtendedSymbol(eosCommon.symbol('FWG', 4)),
          );

        tokens['FWG'].quantity.set_amount(
          fwg.amount.minus(tokens['FWG'].quantity.amount),
        );
    ...
    if (fwf && fwf.amount.gt(0)) {
      if (tokens['FWF']?.quantity.isGreaterThanOrEqual(fwf)) {
        ...
        // нужно обменять с алкора
        if (!tokens['FWF'])
          tokens['FWF'] = new eosCommon.ExtendedAsset(
            0,
            new eosCommon.ExtendedSymbol(eosCommon.symbol('FWF', 4)),
          );

        tokens['FWF'].quantity.set_amount(
          fwf.amount.minus(tokens['FWF'].quantity.amount),
        );
    ...
    if (fww && fww.amount.gt(0)) {
      if (tokens['FWW']?.quantity.isGreaterThanOrEqual(fww)) {
        ...
        // нужно обменять с алкора
        if (!tokens['FWW'])
          tokens['FWW'] = new eosCommon.ExtendedAsset(
            0,
            new eosCommon.ExtendedSymbol(eosCommon.symbol('FWW', 4)),
          );

        tokens['FWW'].quantity.set_amount(
          fww.amount.minus(tokens['FWW'].quantity.amount),
        );
        ...
        actions: [
          {
            ...
            data: {
              ...
              memo: 'deposit',
              quantities: tokens.map((eosAsset) =>
                eosAsset.quantity.toString(),
              ),
              to: 'farmersworld',
            },
          },
  ...
  async withdrawTokens(
    ...
    if (config.fee > options.fee) {
      ...
      this.logger.log(
        `Комиссия на вывод ${config.fee}%. Ожидается меньше ${options.fee}%. Проверка через 10 минут`,
      );
        ...
        (async () => {
          while (true) {
            ...
            try {
              ...
              this.logger.log(
                `Вывод токенов успешно выполнен! [${tokens
